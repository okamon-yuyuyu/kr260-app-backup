...
void print_usage(const char* program) {
    std::cout
        ...
        << "  --model PATH            TorchScript .pt model path\n"
        ...
        << "  --warmup N              warm-up runs (default: 5)\n"
...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--model") {
            ...
            options.torch_threads = std::stoi(require_value());
        } else if (arg == "--warmup") {
            options.warmup_runs = std::stoi(require_value());
    ...
    if (options.torch_threads <= 0) {
        throw std::invalid_argument("torch-threads must be positive");
    }
...
void print_prediction(const PredictionResult& p, const ModelConfig& config) {
    std::cout << "prediction: cmd" << p.command
              ...
              << p.confidence
              << " preprocess=" << std::setprecision(1) << p.preprocess_ms << " ms"
              << " inference=" << p.inference_ms << " ms"
              << " total=" << p.total_ms << " ms\n";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (options.model_path.empty()) {
            std::cout << "TorchScript model path: ";
        ...
        TorchScriptPredictor predictor(options.model_path, options.config_path,
        ...
        std::cout << "\nModel loaded\n"
                  ...
                  << "  torch threads: " << options.torch_threads << '\n'
                  << "  preview: " << (options.preview ? "yes" : "no") << "\n\n";
        std::cout << "  commands:";
        for (int command_id : config.commands) {
            std::cout << ' ' << command_id;
        }
        std::cout << "\n\n";
        ...
        if (!options.preview) {
            ...
            while (true) {
                ...
                if (std::cin >> input_voltage && input_voltage > 0.0) {
        ...
        const std::string run_name =
            options.experiment_name + "_torchscript_predict_" + now_string();
        ...
        while (!stop_requested && index < options.max_steps) {
            ...
            if (aruco.is_goal(aruco_result)) {
                ...
                if (!camera.save_image(image_path, frame)) {
                    throw std::runtime_error("failed to save ArUco goal image");
                }
            ...
            int posture_after_done = current_posture;
            if (1 <= prediction.command && prediction.command <= 9) {
                ...
                posture_after_done = predicted_posture;
            ...
            if (done) {
                current_posture = posture_after_done;
            }
            logger.log(index, timestamp, image_path, prediction,
